...
// ColorWithAlpha returns new color with different alpha
func ColorWithAlpha(c Color, a uint8) Color {
	c.A = a
	return c
}

// IsTransparent returns whether the color is fully transparent
func IsTransparent(c Color) bool {
	return c.A == 0
}

// ColorAsFloat returns RGBA scaled to 0..1
func ColorAsFloat(c Color) (r, g, b, a float32) {
	return float32(c.R) / 0xFF, float32(c.G) / 0xFF, float32(c.B) / 0xFF, float32(c.A) / 0xFF
}

// ColorBytes returns []byte{R, G, B, A}
func ColorBytes(c Color) []byte { return []byte{c.R, c.G, c.B, c.A} }

// Lerp linearly interpolates each RGBA component separately
func ColorLerp(a, b Color, p float32) Color {
	ar, ag, ab, aa := ColorAsFloat(a)
	br, bg, bb, ba := ColorAsFloat(b)
	return RGBA(
		LerpClamp(ar, br, p),
		LerpClamp(ag, bg, p),
		LerpClamp(ab, bb, p),
